...
class ProactiveCandidateGenerationControlService:
    def __init__(self, *, connection_factory: DatabaseConnectionFactory | None = None) -> None:
        self._factory = connection_factory or DatabaseConnectionFactory()
        self._service = ProactiveCandidateGenerationService(connection_factory=self._factory)
    ...
    def by_market(self, *, market_id: str, limit: int = 20) -> dict[str, Any]:
        return _envelope(_result_payload("proactive_candidate_generation by-market", self._service.by_market(market_id=market_id, limit=limit)))

    def by_event(self, *, source_event_id: str, limit: int = 50) -> dict[str, Any]:
        return _envelope(_result_payload("proactive_candidate_generation by-event", self._service.by_event(source_event_id=source_event_id, limit=limit)))

    def by_seed(self, *, proactive_candidate_seed_id: str) -> dict[str, Any]:
        return _envelope(_result_payload("proactive_candidate_generation by-seed", self._service.by_seed(proactive_candidate_seed_id=proactive_candidate_seed_id)))


def _result_payload(source: str, result: dict[str, Any]) -> dict[str, Any]:
    rows = result.get("results") or ([result.get("seed")] if result.get("seed") else [])
    return {
        ...
        "result": result,
        "warnings": [] if rows else ["No proactive candidate seed rows matched this query."],
        "errors": [],
    }
...
def _envelope(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    envelope = truth_envelope(
        ...
        source=str(payload.get("source") or "proactive_candidate_generation"),
