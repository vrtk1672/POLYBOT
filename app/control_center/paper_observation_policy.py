...
class PaperObservationPolicyControlService:
    def __init__(self, *, connection_factory: DatabaseConnectionFactory | None = None) -> None:
        self._factory = connection_factory or DatabaseConnectionFactory()
        self._service = PaperObservationPolicyReviewService(connection_factory=self._factory)
    ...
    def refresh(self, *, limit: int = 500, force: bool = False) -> dict[str, Any]:
        ...
        payload["latest_policy_action"] = result
        return _envelope(payload)

    def by_seed(self, *, proactive_candidate_seed_id: str, limit: int = 20) -> dict[str, Any]:
        return _envelope(_result_payload("paper_observation_policy by-seed", self._service.by_seed(proactive_candidate_seed_id=proactive_candidate_seed_id, limit=limit)))

    def by_market(self, *, market_id: str, limit: int = 20) -> dict[str, Any]:
        return _envelope(_result_payload("paper_observation_policy by-market", self._service.by_market(market_id=market_id, limit=limit)))
...
def _result_payload(source: str, result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "result": result,
        "warnings": [] if rows else ["No Paper Observation policy review rows matched this query."],
        "errors": [],
    }
...
def _envelope(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    envelope = truth_envelope(
        ...
        source=str(payload.get("source") or "paper_observation_policy"),
