...
class RankingV2RunsRepository:
    def open_run(self, conn: Connection, run: RankingV2RunOpenContract) -> None:
        conn.execute(
            """
    ...
    def close_run(self, conn: Connection, run: RankingV2RunCloseContract) -> None:
        conn.execute(
            """
            UPDATE ranking_v2_runs
            SET status = %s,
                ended_at = %s,
                success_count = %s,
                failure_count = %s,
