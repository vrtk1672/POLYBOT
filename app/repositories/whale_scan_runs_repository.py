...
class WhaleScanRunsRepository:
    def open_run(self, conn: Connection, run: WhaleScanRunOpenContract) -> None:
        conn.execute(
            """
            INSERT INTO whale_scan_runs (
                id, source_type, source_ref, status, scanner_version,
    ...
    def close_run(self, conn: Connection, run: WhaleScanRunCloseContract) -> None:
        conn.execute(
            """
            UPDATE whale_scan_runs
            SET status = %s,
                ended_at = %s,
                success_count = %s,
                failure_count = %s,
