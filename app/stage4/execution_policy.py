...
def _extract_exposure_type(order: dict[str, Any]) -> str:
    for key in ("exposure_type", "exposureType"):
        value = order.get(key)
        if value:
...
def evaluate_execution_policy(
    ...
    *,
    ...
    capacity_mode: str = "live",
) -> ExecutionPolicyDecision:
    reasons: list[str] = []
    current_time = now or datetime.now(UTC)
    normalized_capacity_mode = str(capacity_mode).strip().lower()
    max_open_positions = (
        settings.paper_safe_max_open_positions
        if normalized_capacity_mode == "paper_safe"
        else settings.live_max_open_positions
    )
    ...
    active_order_count = sum(1 for order in open_orders if _extract_exposure_type(order) != "POSITION")
    active_position_count = sum(1 for order in open_orders if _extract_exposure_type(order) == "POSITION")
    if active_order_count >= max_open_positions:
        reasons.append(
            f"open orders {active_order_count} meet/exceed {open_orders_label} {max_open_positions}"
        )
    if active_position_count >= max_open_positions:
        reasons.append(
            f"open positions {active_position_count} meet/exceed {open_positions_label} {max_open_positions}"
        )

    same_market_count = 0
    same_market_position_count = 0
    latest_open_order_at: datetime | None = None
    for order in open_orders:
        market_id = _extract_market_id(order)
        exposure_type = _extract_exposure_type(order)
    ...
    if not settings.live_allow_scaling and same_market_position_count > 0:
        reasons.append("ALLOW_SCALING is false and an open live position already exists for this market")
