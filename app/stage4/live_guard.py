...
class LiveGuard:
    ...
    def evaluate_order(
        ...
        *,
        ...
        live: bool,
        armed: bool,
        enforce_notional_cap: bool = True,
    ) -> LiveGuardDecision:
        reasons: list[str] = []
        ...
        if enforce_notional_cap and notional_usd > self.settings.live_max_order_usd:
            reasons.append(
                f"order notional ${notional_usd:.2f} exceeds LIVE_MAX_ORDER_USD ${self.settings.live_max_order_usd:.2f}"
            )
        if live:
            if not self.settings.live_market_whitelist:
                reasons.append("LIVE_MARKET_WHITELIST must be non-empty in live mode")
            elif market_id not in self.settings.live_market_whitelist:
