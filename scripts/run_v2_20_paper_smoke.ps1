...
try {
    ...
    try {
        Invoke-RestMethod -Method Post -Uri "$BaseUrl/runtime/mode/request" -ContentType "application/json" -Body (@{
            to_mode = "PAPER"
            actor = "v2_20_smoke"
            reason = "V2.20 PAPER smoke verification with live disabled"
    ...
    python -m uv run python -m app.tools.v2_20_full_system_run run-smoke --mode PAPER --run-type paper_smoke --duration-seconds $DurationSeconds --interval-seconds $IntervalSeconds --base-url $BaseUrl
} finally {
