    ...
    with DatabaseConnectionFactory().connect() as conn, conn.transaction():
        ...
        SourceStatusRepository().upsert_status(
            conn,
            {
                ...
                "details_json": {},
                "latency_ms": 1,
                "notes": "runtime brain api seed",
            },
        )
    RuntimeProducerEvidenceService().run_runtime_evidence_loop(limit=10, apply_evaluations=True)


def test_runtime_brain_run_endpoint_returns_mock_data_false(postgres_test_schema) -> None:
    _prepare()

    with _client() as client:
        response = client.post("/brain/runtime/run", json={"limit": 10, "write_outputs": True})

    payload = response.json()
    assert response.status_code == 200
    assert payload["mock_data"] is False
    assert payload["brain_outputs_created"] == 1
