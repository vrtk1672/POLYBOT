...
from app.services.query.resolution_analysis_query_service import ResolutionAnalysisQueryService
from app.services.recorders.phase1_cycle_persistence import Phase1CyclePersistenceService
from app.services.resolution_analyzer_lite import (
    ResolutionAnalysisRunResult,
    ResolutionAnalyzerLiteService,
    main as resolution_analyzer_main,
)
            ...
            "interpretations": [
                {
                    ...
                    "explanation": {
                        "why": "Confirmed lineup change affects PSG strength.",
                        "ambiguities": [],
                        "contradictions": [],
                    },
                }
...
def _sample_input() -> EventInterpreterInput:
    return EventInterpreterInput(
        ...
        source_ref="tweet:resolution-lite",
...
def _scored_market(*, market_id: str, question: str, slug: str) -> ScoredMarket:
    ...
    return ScoredMarket(
        ...
        reason="seeded for phase3c tests",
        computed_at=now,
    )
...
def _seed_link_candidate() -> tuple[str, str, str]:
    run_migrations()
    ...
    link_result = linker.link_interpretations([interpretation_id], source_type="phase3c_seed")
    ...
    return interpretation_id, str(candidate_row["id"]), str(candidate_row["market_link_run_id"])


def test_resolution_analyzer_migrations_create_tables(postgres_test_schema) -> None:
    run_migrations()
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        tables = conn.execute(
    ...
    assert {"resolution_analysis_runs", "resolution_analyses"} <= table_names


def test_successful_resolution_analysis_persists_correctly(postgres_test_schema) -> None:
    interpretation_id, candidate_id, market_link_run_id = _seed_link_candidate()
    service = ResolutionAnalyzerLiteService(client=FakeAnthropicClient(_success_resolution_response()))

    result = service.analyze_candidates(
        [candidate_id],
        source_type="resolution_test",
        source_ref="phase3c-test",
    ...
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        runs = conn.execute("SELECT * FROM resolution_analysis_runs").fetchall()
        analyses = conn.execute("SELECT * FROM resolution_analyses").fetchall()
        live_orders = conn.execute("SELECT * FROM live_orders").fetchall()
        paper_runs = conn.execute("SELECT * FROM paper_runs").fetchall()

    assert len(runs) == 1
    assert runs[0]["analyzer_version"] == "phase3c-resolution-analyzer-lite-v1"
    assert runs[0]["prompt_version"] == "phase3c-resolution-analyzer-lite-prompt-v1"
    assert runs[0]["model_name"] == "claude-opus-4-6"

    assert len(analyses) == 1
    analysis = analyses[0]
    assert str(analysis["interpretation_id"]) == interpretation_id
    assert str(analysis["market_link_candidate_id"]) == candidate_id
    assert analysis["market_id"] == "566136"
    assert analysis["market_question"] == "Will PSG beat Marseille tonight?"
    assert analysis["status"] == "SUCCESS"
    assert analysis["direct_fit_class"] == "DIRECT_FIT"
    assert analysis["usable_now_class"] == "USABLE_NOW"
...
def test_malformed_model_output_is_recorded_truthfully(postgres_test_schema) -> None:
    _, candidate_id, market_link_run_id = _seed_link_candidate()
    service = ResolutionAnalyzerLiteService(client=FakeAnthropicClient('{"analyses": ['))

    result = service.analyze_candidates(
        [candidate_id],
        source_type="resolution_test",
        source_ref="phase3c-bad",
        market_link_run_id=market_link_run_id,
    )
    assert result is not None
    ...
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        run_row = conn.execute("SELECT * FROM resolution_analysis_runs LIMIT 1").fetchone()
        analysis_row = conn.execute("SELECT * FROM resolution_analyses LIMIT 1").fetchone()

    assert run_row is not None
    assert run_row["status"] == "FAILED"
    assert analysis_row is not None
    assert analysis_row["status"] == "PARSE_ERROR"
    assert analysis_row["resolution_summary"] is None
    assert analysis_row["error_text"] is not None
...
def test_query_layer_returns_coherent_results(postgres_test_schema) -> None:
    ...
    queries = ResolutionAnalysisQueryService()
    summary = queries.get_resolution_analysis_run_summary(result.resolution_analysis_run_id)
    assert summary is not None
    assert summary["analysis_count"] == 1
    assert summary["status_counts"]["SUCCESS"] == 1

    analyses = queries.list_resolution_analyses_for_run(result.resolution_analysis_run_id)
    assert len(analyses) == 1
    analysis_id = str(analyses[0]["id"])
    details = queries.get_resolution_analysis_details(analysis_id)
    assert details is not None
    assert details["model_name"] == "claude-opus-4-6"

    by_market = queries.list_resolution_analyses_for_market("566136", limit=5)
    assert len(by_market) == 1

    comparison = queries.compare_resolution_analysis_to_link_candidate(analysis_id)
    assert comparison is not None
    assert comparison["candidate"] is not None
    assert comparison["candidate"]["market_id"] == "566136"
...
def test_safe_entry_point_works(postgres_test_schema, monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    ...
    class FakeAnalyzerService:
        def analyze_candidates(self, candidate_ids, *, source_type: str, source_ref: str | None = None):  # noqa: ANN001
            called["candidate_ids"] = candidate_ids
            called["source_type"] = source_type
            called["source_ref"] = source_ref
            return ResolutionAnalysisRunResult(
                resolution_analysis_run_id="resolution-run-cli-test",
                status="COMPLETED",
                input_count=len(candidate_ids),
                success_count=len(candidate_ids),
                failure_count=0,
            )

        def analyze_market_link_run(self, market_link_run_id: str, *, source_ref: str | None = None):  # noqa: ANN001
            called["market_link_run_id"] = market_link_run_id
            called["source_ref"] = source_ref
            return ResolutionAnalysisRunResult(
                resolution_analysis_run_id="resolution-run-cli-test",
                status="COMPLETED",
    ...
    monkeypatch.setattr("app.services.resolution_analyzer_lite.ResolutionAnalyzerLiteService", FakeAnalyzerService)

    exit_code = resolution_analyzer_main(
        ["--market-link-candidate-ids", "candidate-1", "--source-ref", "cli-test"]
    )
    output = capsys.readouterr().out

    assert exit_code == 0
    assert called["candidate_ids"] == ["candidate-1"]
    assert called["source_ref"] == "cli-test"
    assert "resolution-run-cli-test" in output


def test_execution_paths_are_untouched(postgres_test_schema) -> None:
    _, candidate_id, market_link_run_id = _seed_link_candidate()
    service = ResolutionAnalyzerLiteService(client=FakeAnthropicClient(_success_resolution_response()))
    service.analyze_candidates([candidate_id], source_type="isolation_test", market_link_run_id=market_link_run_id)
