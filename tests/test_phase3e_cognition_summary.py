...
from app.services.cognition_summary import (
    CognitionSummaryRunResult,
    CognitionSummaryService,
    main as cognition_summary_main,
)
...
from app.services.query.cognition_summary_query_service import CognitionSummaryQueryService
...
def _success_cognition_summary_response() -> str:
    return json.dumps(
        {
            "summaries": [
                {
                    "narration_summary": "The event is directly linked to the PSG market, the wording fit remains safe, and the main effect is a watchful confidence downgrade rather than thesis invalidation.",
                    "concise_narration_text": "PSG injury news fits the market cleanly, but it weakens confidence enough to keep this market under watch.",
                    ...
                    "evidence": {
                        "event_takeaway": "Confirmed striker absence materially affects PSG strength.",
                        "link_basis": "Interpreter hint matched the persisted PSG-Marseille market directly.",
                        "resolution_takeaway": "Market wording still fits the event safely.",
                        "invalidation_takeaway": "Confidence should degrade, but the thesis is not invalidated.",
                        "open_questions": ["Whether further lineup degradation emerges before kickoff."],
                    },
                }
            ]
        }
...
def _sample_input() -> EventInterpreterInput:
    return EventInterpreterInput(
        ...
        source_ref="tweet:cognition-summary",
...
def _scored_market(*, market_id: str, question: str, slug: str) -> ScoredMarket:
    ...
    return ScoredMarket(
        ...
        reason="seeded for phase3e tests",
        computed_at=now,
    )
...
def _seed_invalidation_reasoning() -> tuple[str, str, str, str]:
    run_migrations()
    ...
    link_result = linker.link_interpretations([interpretation_id], source_type="phase3e_seed")
    ...
    resolution_analysis_id = str(analysis_row["id"])
    ...
    with factory.connect() as conn:
        reasoning_row = conn.execute(
            "SELECT id FROM invalidation_reasonings LIMIT 1"
        ).fetchone()
    ...
    return (
        ...
        candidate_id,
        resolution_analysis_id,
        str(reasoning_row["id"]),
    )


def test_cognition_summary_migrations_create_tables(postgres_test_schema) -> None:
    run_migrations()
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        tables = conn.execute(
    ...
    assert {"cognition_summary_runs", "cognition_summaries"} <= table_names


def test_successful_cognition_summary_persists_correctly(postgres_test_schema) -> None:
    interpretation_id, candidate_id, resolution_analysis_id, invalidation_reasoning_id = _seed_invalidation_reasoning()
    service = CognitionSummaryService(client=FakeAnthropicClient(_success_cognition_summary_response()))

    result = service.summarize_reasonings(
        [invalidation_reasoning_id],
        source_type="cognition_summary_test",
        source_ref="phase3e-test",
    )
    ...
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        runs = conn.execute("SELECT * FROM cognition_summary_runs").fetchall()
        rows = conn.execute("SELECT * FROM cognition_summaries").fetchall()
        live_orders = conn.execute("SELECT * FROM live_orders").fetchall()
        paper_orders = conn.execute("SELECT * FROM paper_orders").fetchall()

    assert len(runs) == 1
    assert runs[0]["narrator_version"] == "phase3e-cognition-summary-v1"
    assert runs[0]["prompt_version"] == "phase3e-cognition-summary-prompt-v1"
    assert runs[0]["model_name"] == "claude-opus-4-6"

    assert len(rows) == 1
    summary = rows[0]
    assert str(summary["interpretation_id"]) == interpretation_id
    assert str(summary["market_link_candidate_id"]) == candidate_id
    assert str(summary["resolution_analysis_id"]) == resolution_analysis_id
    assert str(summary["invalidation_reasoning_id"]) == invalidation_reasoning_id
    assert summary["market_id"] == "566136"
    assert summary["market_question"] == "Will PSG beat Marseille tonight?"
    assert summary["event_summary_snapshot"] == "A key PSG striker is unavailable tonight due to injury."
    assert summary["status"] == "SUCCESS"
    assert summary["cognition_conclusion_class"] == "WATCHFUL"
    assert summary["usability_class"] == "NEEDS_CONFIRMATION"
    assert summary["recommended_operator_focus"] == "MONITOR"
    assert live_orders == []
    assert paper_orders == []


def test_malformed_model_output_is_recorded_truthfully(postgres_test_schema) -> None:
    _, _, _, invalidation_reasoning_id = _seed_invalidation_reasoning()
    service = CognitionSummaryService(client=FakeAnthropicClient('{"summaries": ['))

    result = service.summarize_reasonings(
        [invalidation_reasoning_id],
        source_type="cognition_summary_test",
        source_ref="phase3e-bad",
    )
    assert result is not None
    ...
    factory = DatabaseConnectionFactory()
    with factory.connect() as conn:
        run_row = conn.execute("SELECT * FROM cognition_summary_runs LIMIT 1").fetchone()
        summary_row = conn.execute("SELECT * FROM cognition_summaries LIMIT 1").fetchone()

    assert run_row is not None
    assert run_row["status"] == "FAILED"
    assert summary_row is not None
    assert summary_row["status"] == "PARSE_ERROR"
    assert summary_row["narration_summary"] is None
    assert summary_row["error_text"] is not None
...
def test_query_layer_returns_coherent_results(postgres_test_schema) -> None:
    ...
    queries = CognitionSummaryQueryService()
    summary = queries.get_cognition_summary_run_summary(result.cognition_summary_run_id)
    assert summary is not None
    assert summary["summary_count"] == 1
    assert summary["status_counts"]["SUCCESS"] == 1

    rows = queries.list_cognition_summaries_for_run(result.cognition_summary_run_id)
    assert len(rows) == 1
    summary_id = str(rows[0]["id"])
    details = queries.get_cognition_summary_details(summary_id)
    assert details is not None
    assert details["model_name"] == "claude-opus-4-6"

    by_market = queries.list_cognition_summaries_for_market("566136", limit=5)
    assert len(by_market) == 1

    comparison = queries.compare_cognition_summary_to_invalidation_reasoning(summary_id)
    assert comparison is not None
    assert comparison["invalidation_reasoning"] is not None
    assert comparison["invalidation_reasoning"]["market_id"] == "566136"
...
def test_safe_entry_point_works(postgres_test_schema, monkeypatch: pytest.MonkeyPatch, capsys) -> None:
    ...
    class FakeNarrationService:
        def summarize_reasonings(self, reasoning_ids, *, source_type: str, source_ref: str | None = None):  # noqa: ANN001
            called["reasoning_ids"] = reasoning_ids
            called["source_type"] = source_type
            called["source_ref"] = source_ref
            return CognitionSummaryRunResult(
                cognition_summary_run_id="cognition-summary-cli-test",
                status="COMPLETED",
                input_count=len(reasoning_ids),
                success_count=len(reasoning_ids),
                failure_count=0,
            )

    monkeypatch.setattr("app.services.cognition_summary.CognitionSummaryService", FakeNarrationService)

    exit_code = cognition_summary_main(
        ["--invalidation-reasoning-ids", "reasoning-1", "--source-ref", "cli-test"]
    )
    output = capsys.readouterr().out

    assert exit_code == 0
    assert called["reasoning_ids"] == ["reasoning-1"]
    assert called["source_ref"] == "cli-test"
    assert "cognition-summary-cli-test" in output


def test_execution_paths_are_untouched(postgres_test_schema) -> None:
    _, _, _, invalidation_reasoning_id = _seed_invalidation_reasoning()
    service = CognitionSummaryService(client=FakeAnthropicClient(_success_cognition_summary_response()))
    service.summarize_reasonings(
        [invalidation_reasoning_id],
