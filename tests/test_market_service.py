...
class FakeRuntimeIntelligenceService:
    def __init__(self) -> None:
        self.calls: list[dict[str, object]] = []

    def refresh(self, *, cycle_id: str | None, scored_markets: list[object]) -> None:
        self.calls.append({"cycle_id": cycle_id, "count": len(scored_markets)})
...
async def test_market_service_normalizes_and_ranks_top_n() -> None:
    ...
    intelligence = FakeRuntimeIntelligenceService()
    service = MarketService(
        settings=settings,
        gamma_client=FakeGammaClient([sample_event()]),
    ...
    assert intelligence.calls[0]["count"] == 2
