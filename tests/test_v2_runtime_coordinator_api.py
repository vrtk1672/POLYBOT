    ...
    with DatabaseConnectionFactory().connect() as conn, conn.transaction():
        ...
        SourceStatusRepository().upsert_status(
            conn,
            {
                ...
                "details_json": {},
                "latency_ms": 1,
                "notes": "runtime coordinator api seed",
            },
        )
...
def test_runtime_coordinator_run_endpoint_returns_mock_data_false(postgres_test_schema) -> None:
    _prepare()

    with _client() as client:
        response = client.post("/coordinator/runtime/run", json={"limit": 10, "write_decisions": True})

    payload = response.json()
    assert response.status_code == 200
    assert payload["mock_data"] is False
    assert payload["coordinator_decisions_created"] == 1
