...
from app.services.capital_allocator import CapitalAllocator, CapitalSnapshot, LiveCapitalSource, PaperCapitalSource
...
def test_paper_capital_source_uses_configured_starting_capital(postgres_test_schema) -> None:
    run_migrations()
    settings = Stage4Settings(
        _env_file=None,
        PAPER_STARTING_CAPITAL_USD=250,
        PAPER_MIN_CASH_RESERVE_PCT=0.20,
        PAPER_MAX_ALLOC_PER_TRADE_PCT=0.25,
        PAPER_MAX_TOTAL_DEPLOYMENT_PCT=0.75,
    )
    snapshot = PaperCapitalSource(stage4_settings=settings).snapshot()

    assert snapshot.source_mode == "paper"
    assert snapshot.total_equity_usd == 250.0
    assert snapshot.available_cash_usd == 250.0
    assert snapshot.metadata["reserve_target_usd"] == 50.0
    assert snapshot.metadata["max_alloc_per_trade_usd"] == 62.5
    assert snapshot.metadata["max_total_deployment_usd"] == 187.5


def test_live_capital_source_does_not_fall_back_to_fixed_paper_capital() -> None:
    settings = Stage4Settings(
        _env_file=None,
        PAPER_STARTING_CAPITAL_USD=999,
    ...
    snapshot = LiveCapitalSource(stage4_settings=settings).snapshot()
...
def test_capital_allocator_refuses_entries_when_capital_is_insufficient() -> None:
    ...
    snapshot = CapitalSnapshot(
        ...
        reserved_cash_usd=20.0,
        deployed_notional_usd=55.0,
        ...
        metadata={
            "reserve_target_usd": 20.0,
            "max_alloc_per_trade_usd": 25.0,
            "max_total_deployment_usd": 75.0,
        },
    )

    decision = allocator.plan_entry(
        snapshot=snapshot,
        market_id="mkt-1",
        total_rank=80.0,
        confidence=0.75,
    ...
    assert decision.action == "RESERVE_ONLY"
    assert decision.approved_notional_usd == 0.0
    assert decision.reason_code == "cash_reserved"


def test_capital_allocator_respects_reserve_and_max_alloc_per_trade() -> None:
    allocator = CapitalAllocator(
        Stage4Settings(
            _env_file=None,
            PAPER_STARTING_CAPITAL_USD=100,
    ...
    assert decision.approved_notional_usd == 25.0
    reserved_snapshot = allocator.reserve_pending_allocation(snapshot, approved_notional_usd=decision.approved_notional_usd)
    assert reserved_snapshot.available_cash_usd == 75.0
    assert reserved_snapshot.pending_notional_usd == 25.0
...
def test_execution_policy_does_not_treat_existing_position_as_open_order_limit() -> None:
    scored = [
        _make_scored_market(
            market_id="566136",
            question="PSG",
    ...
    settings = Stage4Settings(_env_file=None, LIVE_MARKET_WHITELIST="", LIVE_MAX_OPEN_POSITIONS=2)
    allowed, _ = build_allowed_universe(scored, recs, settings)
    ranked = rank_candidates(allowed)
    decision = evaluate_execution_policy(
        ranked[0],
        open_orders=[{"market_id": "other-market", "exposure_type": "POSITION"}],
        settings=settings,
    )
    assert decision.allowed is True


def test_execution_policy_uses_paper_safe_capacity_without_changing_live_limits() -> None:
    scored = [
        _make_scored_market(
            market_id="566136",
            question="PSG",
    ...
    live_decision = evaluate_execution_policy(
        ranked[0],
        open_orders=[{"market_id": "other-market", "exposure_type": "POSITION"}],
        settings=settings,
    )
    paper_decision = evaluate_execution_policy(
        ranked[0],
        open_orders=[{"market_id": "other-market", "exposure_type": "POSITION"}],
        settings=settings,
    ...
    assert any("MAX_CONCURRENT_POSITIONS 1" in reason for reason in live_decision.reasons)
    assert paper_decision.allowed is True


def test_execution_policy_keeps_same_market_guard_in_paper_safe_mode() -> None:
    scored = [
        _make_scored_market(
            market_id="553866",
            question="Spurs",
    ...
    decision = evaluate_execution_policy(
        ranked[0],
        open_orders=[{"market_id": "553866", "exposure_type": "POSITION"}],
        settings=settings,
        capacity_mode="paper_safe",
    )
...
async def test_live_mode_submits_only_best_candidate_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: Stage4Settings(
            ...
            LIVE_MARKET_WHITELIST="553866,564198,566136",
        ),
    )
...
async def test_live_mode_falls_back_when_top_candidate_fails_minimum_size(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: Stage4Settings(
            ...
            LIVE_KILL_SWITCH=False,
            LIVE_MAX_ORDER_USD=1,
            LIVE_MARKET_WHITELIST="564198,600001",
        ),
    )
    monkeypatch.setattr(brain, "Stage4ExecutionClient", lambda settings: FakeExecutionClient())
